...
void parse_command(N64Data& input, int cmd[3]) {
  ...
  // Command velocity has the form [start, linear vel, steering angle]
  ...
  if ((input.data1 & BIT(START_BIT)) && !(prev.data1 & BIT(START_BIT))) {
    ...
    cmd[2] = 0;
      ...
      if(abs(input.stick_y) < YMIN) cmd[1] = 0;
      else cmd[1] = (int) map((long) input.stick_y, -YMAX, YMAX, -VEL_MAX, VEL_MAX);
  }
  cmd[0] = start;
  if(abs(input.stick_x) < XMIN) cmd[2] = 0;
  else cmd[2] = (int) map((long) input.stick_x, -YMAX, XMAX, -THETA_MAX, THETA_MAX);
  prev = input;
  // For debugging:
//  Serial.print("Start: ");
//  Serial.print(cmd[0]);
